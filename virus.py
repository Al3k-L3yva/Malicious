...
class ViralSimulator:
    ...
    def change_cursor_size(self):
        ...
        while self.running:
            # Alterna entre tamaños grandes y pequeños
            for size in [32, 48, 64, 32]:
                if not self.running:
                    break
                try:
                    ctypes.windll.user32.SystemParametersInfoW(0x102B, 0, size, 0)  # SPI_SETCURSORS
                except:
                    pass
                time.sleep(0.5)
        ...
        while self.running:
            if self.windows:
                ...
                try:
                    ...
                    new_x = random.randint(0, self.screen_width - new_width)
                    new_y = random.randint(0, self.screen_height - new_height)
    ...
    def run(self):
        ...
        threads = [
            threading.Thread(target=self.move_mouse_randomly),
            threading.Thread(target=self.shake_mouse),
            ...
            # threading.Thread(target=self.change_cursor_size),  # Descomentar en Windows
        ]
